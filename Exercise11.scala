...
/*    val calc1 = datadf.withColumn("TotalFee", round(expr("(Fee * 18.367)/100 + Fee"), 2)).select("*")
    calc1.show()

    val calc4 = calc1.selectExpr("bround(TotalFee, 1)").show()

    val calc2 = datadf.selectExpr("*", "(Fee * 18.367)/100 + Fee as TotalFee").show()

    val calc3 = datadf.selectExpr("*", "(Fee * 18.367)/100 + Fee as TotalFee").show()*/

    //describe is used to give count, max, min, mean, stdev for the column
    val calc5 = datadf.describe("fee").show()
